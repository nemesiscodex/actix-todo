use crate::models::{CreateTodoList, CreateTodoItem, Status, ResultResponse, AppState};
use crate::db;

use actix_web::{web, Responder, HttpResponse};
use deadpool_postgres::{Pool, Client, PoolError};
use crate::errors::AppError;
use slog::{Logger, error, crit, o};
...
pub async fn create_todo(todo_list: web::Json<CreateTodoList>, state: web::Data<AppState>) -> Result<impl Responder, AppError> {
...
pub async fn create_item(list_id: web::Path<(i32,)>, todo_item: web::Json<CreateTodoItem>, state: web::Data<AppState>) -> Result<impl Responder, AppError> {
...
pub async fn items(list_id: web::Path<(i32,)>, state: web::Data<AppState>) -> Result<impl Responder, AppError> {
...
pub async fn get_item(params: web::Path<(i32,i32)>, state: web::Data<AppState>) -> Result<impl Responder, AppError> {
...
pub async fn check_todo(params: web::Path<(i32,i32)>, state: web::Data<AppState>) -> Result<impl Responder, AppError> {
    ...
    result.map(|updated| HttpResponse::Ok().json(ResultResponse{result: updated}))
        .map_err(log_error(sublog))
